...
void recv_thd_client(){
	// recv and push the data into que
	std::cout << "start recving client" << std::endl;
	//recvFromHost();
}

void recv_thd_server(){
	std::cout << "start recving server" << std::endl;
	//recvFromServer();
}

void handle_thd_client(char*& item, u_char type){
	while(true){
		char* tempItem;
		clientQueue.Pop(tempItem);
...
void handle_thd_server(char*& item, u_char type){
	while(true){
		char* tempItem;
		serverQueue.Pop(tempItem);
		auth_header* temp_hdr = (auth_header*)tempItem;
		if(temp_hdr->type == type){
			item = tempItem;
			std::cout << item << std::endl;
